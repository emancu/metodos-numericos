...
El lenguaje utilizado para resolver los algoritmos es C++. En un principio implementamos los algoritmos específicos para cada problema, pero al poco tiempo nos dimos cuenta de que íbamos a repetir el código y sería más difícil el mantenimiento.   
Fue por esto que decidimos implementar una función genérica para cada método, que recibe como parámetro la función a la cual se le quiere aplicar el mismo. Ademas de \textit{Newton} y \textit{Bisección}, decidimos implementar la combinación de los mismos, es decir conseguir una \textit{semilla} con el método de \textbf{bisección} y luego utilizar la misma para aplicar \textbf{Newton}. \\ \hspace{1em}

Los parámetros los pasa el usuario por linea de comandos y son almacenados en una estructura interna.  
...
En el punto dos para calcular la altura máxima luego del primer rebote lo que hicimos fue calcular la raíz a la función velocidad y con este resultado calcular la posición, ya que la altura máxima seria en el momento en que la pelota tiene velocidad nula. En cambio para el segundo impacto calculamos el cero de la función posición, pero cambiando los parámetros iniciales. En el instante del primer rebote la altura de la pelota es 0 y calculamos la velocidad con la que rebota, siendo estos los nuevos parámetros para la función posición.
...
Como la clase TFloat, alcanza su máxima representación con 52 bits de mantisa, este fue el valor que utilizamos para tomar como valor válido. Luego, fuimos decrementando la mantisa y ejecutando el mismo problema pero con menor precisión.
Una vez obtenidos los resultados, los comparamos con nuestro \textit{valor ideal} y representamos la diferencia en el gráfico.
Claramente una precision acotada aumenta el error en los cálculos.
